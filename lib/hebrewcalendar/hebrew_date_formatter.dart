...
class HebrewDateFormatter {
  ...
  Map<Parsha, String> _transliteratedParshaMap = {
    Parsha.NONE: "",
  ...
  void setLongWeekFormat(bool longWeekFormat) {
    this._longWeekFormat = longWeekFormat;
  }
  ...
  void setTransliteratedShabbosDayOfWeek(String transliteratedShabbos) {
    this._transliteratedShabbosDayOfweek = transliteratedShabbos;
  }
  ...
  void setTransliteratedHolidayList(List<String> transliteratedHolidays) {
    this._transliteratedHolidays = _transliteratedHolidays;
  }
  ...
  String formatYomTov(JewishCalendar jewishCalendar) {
    ...
    return index == -1
        ? ""
        : _hebrewFormat ? _hebrewHolidays[index] : _transliteratedHolidays[index];
  }
  ...
  void setHebrewFormat(bool hebrewFormat) {
    this._hebrewFormat = hebrewFormat;
  }
  ...
  void setHebrewOmerPrefix(String hebrewOmerPrefix) {
    this._hebrewOmerPrefix = hebrewOmerPrefix;
  }
  ...
  void setTransliteratedMonthList(List<String> transliteratedMonths) {
    this._transliteratedMonths = _transliteratedMonths;
  }
  ...
  bool isUseGershGershayim() => _useGershGershayim;
  ...
  void setUseGershGershayim(bool useGershGershayim) {
    this._useGershGershayim = useGershGershayim;
  }
  ...
  bool isUseFinalFormLetters() => _useFinalFormLetters;
  ...
  void setUseFinalFormLetters(bool useFinalFormLetters) {
    this._useFinalFormLetters = useFinalFormLetters;
  }
  ...
  bool isUseLongHebrewYears() => _useLonghebrewYears;
  ...
  void setUseLongHebrewYears(bool useLongHebrewYears) {
    this._useLonghebrewYears = useLongHebrewYears;
  }
    ...
    if (isHebrewFormat()) {
      if (jewishDate.isJewishLeapYear() && month == JewishDate.ADAR) {
        return _hebrewMonths[13] +
            (_useGershGershayim ? _GERESH : ""); // return Adar I, not Adar in a leap year
      ...
      if (jewishDate.isJewishLeapYear() && month == JewishDate.ADAR) {
        return _transliteratedMonths[13]; // return Adar I, not Adar in a leap year
      } else {
  ...
  String formatOmer(JewishCalendar jewishCalendar) {
    ...
    if (_hebrewFormat) {
      return formatHebrewNumber(omer) + " " + _hebrewOmerPrefix + "\u05E2\u05D5\u05DE\u05E8";
    } else {
      if (omer == 33) {
        // if lag b'omer
        return "Lag BaOmer";
  ...
  String getLongOmer(JewishCalendar jewishCalendar) {
    int omer = jewishCalendar.getDayOfOmer();
    switch (omer) {
      case 1:
  ...
  String getFormattedKviah(int jewishYear) {
    ...
    returnValue = returnValue.replaceAll(_GERESH, ""); // geresh is never used in the kviah format
  ...
  String formatHebrewNumber(int number) {
    ...
    if (number % 1000 == 0) {
      ...
      sb.write(" ");
      sb.write(ALAFIM); // add # of thousands plus word thousand (overide alafim boolean)
      return sb.toString();
      ...
      if (number % 10 == 0) {
        ...
        if (singleDigitNumber == false) {
          if (isUseFinalFormLetters()) {
            sb.write(jTenEnds[tens]); // years like 5780 will end with a final form &#x05E3;
          } else {
            sb.write(jTens[tens]); // years like 5780 will end with a regular &#x05E4;
          }
        } else {
          sb.write(jTens[tens]); // standard letters so years like 5050 will end with a regular nun
        }
      } else {
        sb.write(jTens[tens]);
        number = number % 10;
  ...
  Map<Parsha, String> getTransliteratedParshiosList() {
    return _transliteratedParshaMap;
  }
  ...
  void setTransliteratedParshiosList(Map<Parsha, String> transliteratedParshaMap) {
    this._transliteratedParshaMap = transliteratedParshaMap;
  }
  ...
  String formatParsha(JewishCalendar jewishCalendar) {
    ...
    return _hebrewFormat ? _hebrewParshaMap[parsha] : _transliteratedParshaMap[parsha];
  }

  String formatWeeklyParsha(JewishCalendar jewishCalendar) {
    int delta = 7 - jewishCalendar.getDayOfWeek();
    DateTime date = DateTime.parse(jewishCalendar.getGregorianCalendar().toIso8601String());
    return formatParsha(JewishCalendar.fromDateTime(date.add(Duration(days: delta))));
  }
  ...
  String formatSpecialParsha(JewishCalendar jewishCalendar) {
    ...
    return _hebrewFormat
        ...
        : _transliteratedParshaMap[specialParsha];
  }
}